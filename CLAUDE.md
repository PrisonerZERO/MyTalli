...
- **`<Version>1.0.0.17</Version>`** in `My.Talli.Web.csproj` — single source of truth for the app version. Format: `Major.Minor.Patch.Revision`. **Initial v1.0.0.0 went live on prod 2026-05-03; subsequent revisions (v1.0.0.1–v1.0.0.16) have shipped via PR + redeploy. v1.0.0.9 (May 17) shipped free-tier enforcement (1 shop, 30-day history, no CSV export) + plan expiration lifecycle UX (banner + email + reconciliation worker). v1.0.0.10 (May 17) added BillingHealthWorker visibility + Send Subscription Expired admin button. v1.0.0.11 (May 17) pre-launch perf pass — server-side CountAsync + IsProSubscriberCommand per-circuit cache + initial nav loading bar. v1.0.0.12 (May 17) wired IDbContextFactory for true parallel queries on Export preview. v1.0.0.13 (May 17) replaced the subtle nav bar with a "Working…" modal dialog (2s minimum, MutationObserver guards the active class against Blazor's enhanced-nav DOM merge). v1.0.0.14 (May 17 PM) made Dashboard sample-data banner mirror Goals (two CTAs). v1.0.0.15 (May 17 PM) rewrote Stripe Connect from Account Links to OAuth Connect — 🚨 prod token-exchange currently failing, site in MM pending diagnosis. v1.0.0.16 (May 18) whitelisted the landing page (`/`), sign-in routes (`/signin*`), and auth endpoints (`/api/auth*`) in MM so anyone can reach them while MM is on. v1.0.0.17 (May 18, local only on `development_ROBERT`) whitelisted all static assets (any path matched by `Path.HasExtension`) so the landing page renders styled while MM is on (without this, `/app.css`, `/My.Talli.Web.styles.css`, `/favicon.svg` were redirected to `/maintenance` and the page rendered raw).**
...
- **Live:** `https://www.mytalli.com/` — initial **v1.0.0.0** shipped 2026-05-03 via VS Publish ZipDeploy → staging slot → slot swap. Subsequent revisions (v1.0.0.1 through **v1.0.0.16** — May 18 Maintenance Mode whitelist for landing + sign-in routes) have shipped via PR-to-`main` + redeploy. **v1.0.0.17 is committed locally on `development_ROBERT` and not yet merged/deployed** — it extends the MM whitelist to all static assets so the landing page renders styled while MM is on. Every release cycle now: develop on `development_ROB`/`_ROBERT` (name alternates), PR to `main`, merge, push to Azure.
...
- **Stripe Connect uses OAuth, same pattern as Etsy and Gumroad** (v1.0.0.15 — flipped from the earlier Account Links flow). Endpoints are `/api/platforms/stripe/connect` (builds the `connect.stripe.com/oauth/authorize` URL with `client_id=ca_xxx&scope=read_write&state=…&redirect_uri=…`, stores state in an encrypted cookie, redirects), `/api/platforms/stripe/reconnect/{shopConnectionId}` (ownership-check + same authorize-URL redirect for token-refresh flows), and `/api/platforms/stripe/callback` (validates state, POSTs to `connect.stripe.com/oauth/token` with `client_secret = platform Stripe.SecretKey`, gets back `access_token + stripe_user_id + refresh_token`, fetches the account profile, runs `ConnectStripeCommand`). The OAuth `client_secret` is the platform's existing `Stripe.SecretKey` — only a `ClientId` (`ca_xxx` from the Stripe Connect application) is configured separately. `ShopConnection.AccessToken` holds the OAuth access token (acts as a per-account API key — `RequestOptions.ApiKey = accessToken` on all reads). **Scope note:** Stripe gates the `read_only` scope behind a support ticket; new platforms can only use `read_write` out of the box. The code's default in `StripeConnectSettings` is `"read_only"` but production has `StripeConnect__Scope=read_write` set as an Azure env var override. There's an open cleanup item to flip the code default to `read_write` so the env var override isn't needed. **Why OAuth instead of Account Links:** MyTalli is a read-only revenue aggregator — we need to read a user's existing Stripe account, not create a brand-new connected account for them. The previous Account Links flow forced users through full Stripe Standard onboarding (business profile, identity, bank, 2FA) for an account they had no use for, while the user's existing Stripe data stayed unconnected. OAuth Connect ("Sign in with your existing Stripe account → approve read-only access") is the correct pattern for our use case. **🚨 KNOWN BLOCKER (2026-05-17 PM, unresolved):** the OAuth flow correctly reaches Stripe's consent page in production with the right client_id / redirect_uri / scope, BUT the token exchange in `/callback` throws — users always land on `/platforms?error=stripe_exchange` even after approving the OAuth grant on a fully-verified Stripe account. Site put into Maintenance Mode while bug is investigated. Diagnosis path documented in `memory/project_stripe_setup.md` (must capture actual exception from Azure Log Stream → fix → redeploy v1.0.0.16).
