...
- **Free-tier 1-shop-per-platform cap.** Free-tier users are capped at one shop *per platform*; Pro users (ProductId 1 or 2, `Active` or `Cancelling`) are uncapped. Enforcement is two-layer:
  ...
  - **Server:** Each platform's Connect endpoint (today only [EtsyConnect](Source/My.Talli.Web/Endpoints/PlatformEndpoints.cs); Stripe/Gumroad/PayPal/Shopify follow the same template when added) runs the Pro/shop-count check and refuses the OAuth launch if the free-tier user already has 1 shop on that platform. Redirects to `/platforms?error=plan_limit`, which the view model maps to an explanatory toast. Defense-in-depth — UI hides the trigger, server refuses the request.
