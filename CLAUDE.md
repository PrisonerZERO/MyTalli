...
- **`<Version>1.0.0.18</Version>`** in `My.Talli.Web.csproj` — single source of truth for the app version. Format: `Major.Minor.Patch.Revision`. **Initial v1.0.0.0 went live on prod 2026-05-03; subsequent revisions (v1.0.0.1–v1.0.0.18) ship via PR + redeploy. v1.0.0.9 (May 17) shipped free-tier enforcement (1 shop, 30-day history, no CSV export) + plan expiration lifecycle UX (banner + email + reconciliation worker). v1.0.0.10 (May 17) added BillingHealthWorker visibility + Send Subscription Expired admin button. v1.0.0.11 (May 17) pre-launch perf pass — server-side CountAsync + IsProSubscriberCommand per-circuit cache + initial nav loading bar. v1.0.0.12 (May 17) wired IDbContextFactory for true parallel queries on Export preview. v1.0.0.13 (May 17) replaced the subtle nav bar with a "Working…" modal dialog (2s minimum, MutationObserver guards the active class against Blazor's enhanced-nav DOM merge). v1.0.0.14 (May 17 PM) made Dashboard sample-data banner mirror Goals (two CTAs). v1.0.0.15 (May 17 PM) rewrote Stripe Connect from Account Links to OAuth Connect — production token-exchange callback failed post-deploy, site put into Maintenance Mode while bug was diagnosed. v1.0.0.16 (May 18) whitelisted the landing page (`/`), sign-in routes (`/signin*`), and auth endpoints (`/api/auth*`) in MM so anyone can reach them while MM is on. v1.0.0.17 (May 18) whitelisted all static assets (any path matched by `Path.HasExtension`) so the landing page renders styled while MM is on (without this, `/app.css`, `/My.Talli.Web.styles.css`, `/favicon.svg` were redirected to `/maintenance` and the page rendered raw). v1.0.0.18 (May 29) fixed the v1.0.0.15 Stripe Connect blocker: the token exchange itself was succeeding, but the next call (`GetAccountAsync`) threw 404 because Stripe.net's `AccountService.GetAsync(id: null, ...)` constructs `GET /v1/accounts/` (trailing slash, empty id). Fix passes `tokenResponse.StripeUserId` so the SDK builds `GET /v1/accounts/{id}` correctly. Reproduced and verified locally on read_only scope before deploy.**
