...
- **`<Version>1.0.0.8</Version>`** in `My.Talli.Web.csproj` — single source of truth for the app version. Format: `Major.Minor.Patch.Revision`. **Initial v1.0.0.0 went live on prod 2026-05-03; subsequent revisions (v1.0.0.1–v1.0.0.8) have shipped via PR + redeploy. v1.0.0.7 (May 16) bundled the silent-data-loss sweep (Stripe + Gumroad sync DateTime Kind, Module checkout fallback) + the project-wide UTC DateTime value converter on `TalliDbContext` + sidebar UX polish. v1.0.0.8 (May 16) added SQL-backed DataProtection (new `components.DataProtectionKey` table) and removed the `IShopTokenProtector` at-rest encryption layer — OAuth tokens are now stored plaintext on `ShopConnection`.**
...
- **Live:** `https://www.mytalli.com/` — initial **v1.0.0.0** shipped 2026-05-03 via VS Publish ZipDeploy → staging slot → slot swap. Subsequent revisions (v1.0.0.1 through **v1.0.0.8** — May 16 SQL-backed DataProtection + plaintext OAuth tokens) have shipped via PR-to-`main` + redeploy. Every release cycle now: develop on `development_ROB`/`_ROBERT` (name alternates), PR to `main`, merge, push to Azure.
