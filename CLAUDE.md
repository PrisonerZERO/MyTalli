...
| `components` | Third-party component tables | ELMAH_Error (created by Post-Deployment SQL script), DataProtectionKey (EF-managed; one row per ASP.NET DataProtection key) |
| `dbo` | Reserved (empty) | — |
...
└── Source/
    ...
    │   │   ├── Stripe/                         # Stripe Connect OAuth POCOs (shared with Web's StripeConnectService)
    │   │   │   ├── StripeAccountInfo.cs        # Account-info DTO consumed by ConnectStripeCommand: { AccountId (acct_xxx), BusinessName, Email }. Populated from Stripe.net's Account fetch (GET /v1/account) using the OAuth access token.
    │   │   │   ├── StripeOAuthGenerator.cs     # Static helper: BuildAuthorizeChallenge (state-only, no PKCE) — mirrors the Gumroad pattern; emits the connect.stripe.com/oauth/authorize URL with response_type=code + client_id (ca_xxx) + redirect_uri + scope + state
    │   │   │   └── StripeTokenResponse.cs      # OAuth token exchange response (access_token, refresh_token, stripe_user_id, livemode, scope, token_type, stripe_publishable_key)
    │   │   ├── JsonSerializers/
    │   │   │   └── User/
    ...
    │   │           ├── ConnectStripeCommand.cs       # Upsert PlatformConnection + ShopConnection after Stripe OAuth callback (no subtable — base ShopConnection columns cover Stripe). Stores stripe_user_id (acct_xxx) in PlatformAccountId/PlatformShopId, the OAuth access_token in AccessToken, refresh_token in RefreshToken.
    ...
    │   │           ├── SwapShopAccountCommand.cs     # Reconnect-swap — re-point an existing ShopConnection to a different account (IDOR + platform checked), reset sync state, hard-delete the old account's Revenue/Expense/Payout (subtables cascade); same-account = token refresh only. Refreshes ShopConnectionEtsy detail on Etsy.
    │   │           ├── SwapShopAccountInput.cs        # Input — target shopConnectionId + new account identity/tokens (+ optional EtsyDetail)
    │   │           ├── SwapShopAccountResult.cs       # Result — Found (owned + exists) + Swapped (different account vs same-account refresh)
    │   │           ├── SwapShopEtsyDetail.cs          # Etsy-only shop detail (country/currency/URL/vacation) carried on the swap input
    ...
    │   │       ├── SwapShopAccountCommandTests.cs          # Different-account repoint + sync-state reset, hard-delete shop data, same-account refresh keeps data, IDOR (other user) no-op, cross-platform mismatch no-op, Etsy detail upsert
    ...
    │   │   ├── Stripe/
    │   │   │   └── StripeOAuthGeneratorTests.cs    # State-only authorize URL (connect.stripe.com/oauth/authorize), no PKCE artifacts, randomized state across calls
    │   │   ├── JsonSerializers/
    ...
    │   │       └── StripeApiClientStub.cs      # IStripeConnectApiClient stub — queue-based StripeList<Charge>/StripeList<Payout> responses, captures every call (including the accessToken) for assertions. OAuth endpoints (ExchangeCodeAsync/GetAccountAsync) throw NotImplementedException since sync tests don't exercise them.
    │   ├── Middleware/
    ...
    │           └── StripeConnectSyncServiceTests.cs # Charge + payout pagination via starting_after cursor, dedup across syncs, cents → decimal conversion, refunded/disputed flag passthrough, balance_transaction_id used as canonical dedup key, created-after defaults to 90-day backfill floor, OAuth access token passed through to API client, empty access token throws before any API call
    └── My.Talli.Web/               # Blazor Server web project
        ...
        │   ├── PlatformEndpoints.cs   # /api/platforms/{etsy,gumroad,stripe}/{connect,reconnect/{id},callback} (PKCE/state, data-protected cookie). Reconnect threads shopConnectionId → callback runs SwapShopAccountCommand (refresh-or-swap).
        ...
        │   │   ├── IStripeConnectApiClient.cs    # Interface for the 4 Stripe HTTP calls used (ExchangeCode, GetAccount, ListCharges, ListPayouts) — exists so StripeConnectSyncService can be unit-tested with a stub
        ...
        │   │   ├── StripeConnectService.cs       # Implements IStripeConnectApiClient via Stripe.net SDK + raw HttpClient for the OAuth token endpoint. OAuth Connect flow: BuildAuthorizeChallenge emits the connect.stripe.com/oauth/authorize URL; ExchangeCodeAsync POSTs to connect.stripe.com/oauth/token with the platform Stripe.SecretKey as client_secret. ListCharges/ListPayouts/GetAccount pass RequestOptions.ApiKey = OAuth access_token (the seller's account). No client_secret separate from billing — the platform's existing Stripe.SecretKey doubles as the OAuth client_secret.
        │   │   ├── StripeConnectSettings.cs      # Stripe Connect OAuth config POCO (ClientId = ca_xxx, RedirectUri = .../api/platforms/stripe/callback, Scope = "read_only"). Client secret is NOT here — the platform's existing Stripe.SecretKey is reused as the OAuth client_secret.
        │   │   └── StripeConnectSyncService.cs   # IPlatformSyncService — reads ShopConnection.AccessToken (OAuth access token), throws InvalidOperationException if empty (user must reconnect), paginates GET /v1/charges (with expand=balance_transaction for fee/net) and GET /v1/payouts; dedups by balance_transaction_id (revenue) and stripe_payout_id (payouts). Cents → decimal conversion. No expense sync first pass — Stripe processing fees already captured in Revenue.FeeAmount.
        │   └── Tokens/
        ...
        │   ├── PlatformItem.cs            # Platforms page platform row (nested Shops list; SupportsMultipleShops gates the "Connect another shop" button — false for Gumroad)
...
- **`<Version>1.0.0.21</Version>`** in `My.Talli.Web.csproj` — single source of truth for the app version. Format: `Major.Minor.Patch.Revision`. **Initial v1.0.0.0 went live on prod 2026-05-03; subsequent revisions (v1.0.0.1–v1.0.0.21) ship via PR + redeploy. v1.0.0.9 (May 17) shipped free-tier enforcement (1 shop, 30-day history, no CSV export) + plan expiration lifecycle UX (banner + email + reconciliation worker). v1.0.0.10 (May 17) added BillingHealthWorker visibility + Send Subscription Expired admin button. v1.0.0.11 (May 17) pre-launch perf pass — server-side CountAsync + IsProSubscriberCommand per-circuit cache + initial nav loading bar. v1.0.0.12 (May 17) wired IDbContextFactory for true parallel queries on Export preview. v1.0.0.13 (May 17) replaced the subtle nav bar with a "Working…" modal dialog (2s minimum, MutationObserver guards the active class against Blazor's enhanced-nav DOM merge). v1.0.0.14 (May 17 PM) made Dashboard sample-data banner mirror Goals (two CTAs). v1.0.0.15 (May 17 PM) rewrote Stripe Connect from Account Links to OAuth Connect — production token-exchange callback failed post-deploy, site put into Maintenance Mode while bug was diagnosed. v1.0.0.16 (May 18) whitelisted the landing page (`/`), sign-in routes (`/signin*`), and auth endpoints (`/api/auth*`) in MM so anyone can reach them while MM is on. v1.0.0.17 (May 18) whitelisted all static assets (any path matched by `Path.HasExtension`) so the landing page renders styled while MM is on (without this, `/app.css`, `/My.Talli.Web.styles.css`, `/favicon.svg` were redirected to `/maintenance` and the page rendered raw). v1.0.0.18 (May 29) fixed the v1.0.0.15 Stripe Connect blocker: the token exchange itself was succeeding, but the next call (`GetAccountAsync`) threw 404 because Stripe.net's `AccountService.GetAsync(id: null, ...)` constructs `GET /v1/accounts/` (trailing slash, empty id). Fix passes `tokenResponse.StripeUserId` so the SDK builds `GET /v1/accounts/{id}` correctly. Reproduced and verified locally on read_only scope before deploy. v1.0.0.19 (Jun 5) added a `Console.Error.WriteLine` diagnostic in the Stripe callback catch block (plain `LogError` was silent in Azure Log Stream; App Insights captured the real exception) — confirmed the prod 400 was an account-history block on the old test account, not a code bug. v1.0.0.20 (Jun 5, local — not yet deployed) shipped the platform-connection UX cluster: **Reconnect = account swap** (re-point a shop to a different account + hard-delete the old account's data, chosen over a Disconnect button), the Platforms **Available vs Coming Soon** split (coming-soon platforms no longer counted/labeled as available), and **hiding "Connect another shop" for Gumroad** (`PlatformItem.SupportsMultipleShops`). v1.0.0.21 (Jun 5, local) added friendly Stripe OAuth cancel/error messages (`stripe_denied` / `stripe_invalid` / `stripe_state`) to `PlatformsViewModel` — they previously fell through to the generic "Something went wrong" error, so a user who clicked Stripe's "Return to MyTalli" (cancel) was wrongly told something failed. Now mirrors the Etsy/Gumroad copy ("You cancelled the connection to Stripe. No data was saved.").**
...
- `appsettings.Development.json` is **git-ignored** (`.gitignore:26`) — local dev secrets stay out of the repo. Production reads the same keys from Azure App Service environment variables. If a future collaborator joins, they get the file via a separate secure channel (not through git).
...
- **Live:** `https://www.mytalli.com/` — initial **v1.0.0.0** shipped 2026-05-03 via VS Publish ZipDeploy → staging slot → slot swap. Subsequent revisions (v1.0.0.1 through **v1.0.0.18** — May 29 Stripe Connect `GetAccountAsync` fix) ship via PR-to-`main` + redeploy. **v1.0.0.18 contains the fix for the Stripe Connect token-exchange callback** that put the site in MM since v1.0.0.15 — the failure was actually downstream of the token exchange, in a Stripe.net SDK call constructing a malformed `/v1/accounts/` URL. Every release cycle now: develop on `development_ROB`/`_ROBERT` (name alternates), PR to `main`, merge, push to Azure.
- **DataProtection storage: SQL-backed (v1.0.0.8).** Keys persist to `components.DataProtectionKey` via `Microsoft.AspNetCore.DataProtection.EntityFrameworkCore`. `TalliDbContext` implements `IDataProtectionKeyContext`; `PlatformsConfiguration.cs` registers `.PersistKeysToDbContext<TalliDbContext>()`. Used by **auth cookies + antiforgery + the Stripe Connect onboarding cookie** (IDataProtector round-trip values that need encryption). Survives container restarts, slot swaps, and scale events. **No Managed Identity required, no blob storage, no MSI sidecar dependency** — sidesteps the v1.0.0.0 deploy nightmare entirely. Keys are backed up with regular Azure SQL backups.
- **OAuth tokens are stored plaintext on `app.ShopConnection` (v1.0.0.8).** The earlier `IShopTokenProtector` / `DataProtectionShopTokenProtector` layer was removed as security theater — co-locating the decryption keys with the data in the same SQL DB gave no real defense against a full-DB compromise, while creating a permanent operational tax (lose the keys → every connected platform requires a manual reconnect). `ShopConnection.AccessToken` / `RefreshToken` are now raw plaintext OAuth tokens. The DB itself (Azure SQL, firewall + auth) is the trust boundary. Threat tradeoff: lose protection against partial leaks (screenshot of a single column, narrow read-only grant, accidental log serialization); gain zero-reconnect lifecycle as long as the platform's own refresh token doesn't expire.
- **Migration path (post-deploy of v1.0.0.8):** the `DataProtection__BlobStorage__AccountName` / `__ContainerName` env vars on both slots can be deleted (they're no longer read). The `mytallistorage01/dataprotection-keys` blob container is now unused and can be left in place or decommissioned. Any encrypted OAuth tokens on `app.ShopConnection` from before v1.0.0.8 are ciphertext blobs that the new plaintext-only code path will send AS-IS to the platform API → 401/403 rejection → sync marks shop Failed → user reconnects. One-time tax per shop.
...
- **Per-shop controls, not per-platform.** The shop row on `/platforms` shows **Pause** / **Resume** (toggles `ShopConnection.IsEnabled`) and **Reconnect** (see next bullet). No "Sync Now" — syncing is scheduled server-side via `ShopConnection.NextSyncDateTime`; a manual trigger would undermine the rate-limit strategy. No "Manage" — there's no separate management surface; connect/pause/reconnect on this page is the management surface.
- **Reconnect = refresh OR swap account (the fix for "wrong account connected").** Every connected shop (Etsy, Gumroad, Stripe) shows a **Reconnect** button. It re-runs the platform's OAuth flow targeting the *same shop slot* — the `shopConnectionId` is threaded through the OAuth challenge cookie as `ReconnectShopConnectionId`. The callback then dispatches to `SwapShopAccountCommand` (Domain) instead of the normal Connect command:
  - **Same account re-authorized** (`PlatformShopId` matches) → token refresh only, data untouched, redirect `?{platform}=refreshed`.
  - **Different account authorized** → **swap**: the existing `ShopConnection` row is re-pointed to the new account (tokens, `PlatformAccountId`/`PlatformShopId`, `ShopName`, sync state all reset), and the old account's `Revenue`/`Expense`/`Payout` rows are **hard-deleted** (the per-platform subtables cascade away via their `OnDelete: Cascade` FK), then re-synced fresh. Redirect `?{platform}=swapped`.
  - The slot is **never freed** — the row is re-pointed in place — so this is deliberately NOT a disconnect and creates no free-tier-gaming vector (no cooldown/counter machinery needed). `SwapShopAccountCommand` is IDOR-checked (`UserId`) and platform-checked (loads the parent `PlatformConnection` and verifies `Platform`). For Etsy it also refreshes the `ShopConnectionEtsy` subtable (country/currency/URL/vacation) so the spoke's deep-link doesn't point at the old shop.
  - A **danger-styled `ConfirmDialog`** on `/platforms` (`ReconnectingPlatform` / `StartReconnect` / `ConfirmReconnect` / `CancelReconnect` on `PlatformsViewModel`) warns that signing in with a *different* account permanently replaces the shop's data before the OAuth redirect. This is the chosen fix for the "I connected the wrong account" problem **instead of a Disconnect button** (disconnect was rejected: it frees the slot → reopens the gaming vector + needs abuse guards; swap fixes the same mistake in place).
...
- **Stripe Connect uses OAuth, same pattern as Etsy and Gumroad** (v1.0.0.15 — flipped from the earlier Account Links flow). Endpoints are `/api/platforms/stripe/connect` (builds the `connect.stripe.com/oauth/authorize` URL with `client_id=ca_xxx&scope=read_only&state=…&redirect_uri=…`, stores state in an encrypted cookie, redirects), `/api/platforms/stripe/reconnect/{shopConnectionId}` (ownership-check + same authorize-URL redirect; threads `shopConnectionId` into the cookie so the callback runs the refresh-or-swap path — see the "Reconnect = refresh OR swap account" rule), and `/api/platforms/stripe/callback` (validates state, POSTs to `connect.stripe.com/oauth/token` with `client_secret = platform Stripe.SecretKey`, gets back `access_token + stripe_user_id + refresh_token`, fetches the connected account via `GET /v1/accounts/{stripe_user_id}`, runs `ConnectStripeCommand`). The OAuth `client_secret` is the platform's existing `Stripe.SecretKey` — only a `ClientId` (`ca_xxx` from the Stripe Connect application) is configured separately. `ShopConnection.AccessToken` holds the OAuth access token (acts as a per-account API key — `RequestOptions.ApiKey = accessToken` on all reads). **Scope:** `read_only` enabled on both live AND test/sandbox after Stripe support flipped the Connect application's internal tag from "Connect Platform" to "Connect Extension" (live 2026-05-22, sandbox 2026-05-26). Sandbox required a separate support ticket — Stripe configures test-mode and live-mode scope availability independently. Code default in `StripeConnectSettings` is `"read_only"`; production env var `StripeConnect__Scope` should match (was `read_write` pre-retag; flip to `read_only` after the v1.0.0.18 deploy as cleanup). **Why OAuth instead of Account Links:** MyTalli is a read-only revenue aggregator — we need to read a user's existing Stripe account, not create a brand-new connected account for them. The previous Account Links flow forced users through full Stripe Standard onboarding (business profile, identity, bank, 2FA) for an account they had no use for, while the user's existing Stripe data stayed unconnected. OAuth Connect ("Sign in with your existing Stripe account → approve read-only access") is the correct pattern for our use case. **`GetAccountAsync` SDK trap (fixed v1.0.0.18):** Stripe.net's `AccountService.GetAsync(id: null, ...)` constructs `GET /v1/accounts/` (trailing slash, empty id) and Stripe's API returns 404 *"Unrecognized request URL"*. Always pass the connected account's `stripe_user_id` (available from the OAuth token response) so the SDK builds `GET /v1/accounts/{id}`. The bug masqueraded as a token-exchange failure for 12 days (2026-05-17 → 2026-05-29) because the call sits inside the same `try` block as `ExchangeCodeAsync` — site was in Maintenance Mode the whole window while we diagnosed.
- **Multiple shops per platform — only where it's a real pattern.** Nothing *technical* stops a single MyTalli user from connecting multiple accounts on a platform, but the **"Connect another shop" button is gated by `PlatformItem.SupportsMultipleShops`** (set in `PlatformsViewModel.GetPlatformCatalog()`), not shown blindly for all:
  - **Etsy** (`SupportsMultipleShops = true`) — genuinely multi-shop: Etsy caps one shop per login, so multi-shop sellers run multiple logins. This is the core multi-shop use case.
  - **Gumroad** (`SupportsMultipleShops = false`) — the button is **hidden**. Gumroad has no "shop" concept (the account *is* the seller), multiple Gumroad accounts is rare, and Gumroad's OAuth shows no account switcher (it silently authorizes whatever account the browser is signed into), so the button overpromised something it couldn't smoothly deliver — clicking it usually just re-refreshed the same account. The "wrong account" recovery is handled by **Reconnect-swap** instead.
  - **Stripe / future PayPal / Shopify** default to `true` for now (multiple business accounts is plausible); revisit per-platform if the same friction proves it's not worth it.
  - When the button *is* shown, the per-platform OAuth endpoint still enforces the free-tier cap (see below). The flag is UI-only — it hides the sole UI path; it does not hard-block the connect endpoint.
- **Free-tier 1-shop-total cap.** Free-tier users are capped at **1 shop total across all platforms** (not per platform). Pro users (ProductId 1 or 2, `Active` or `Cancelling`, with `EndDate >= UtcNow`) are uncapped. The "free slot" is the user's **oldest** `ShopConnection` (lowest `Id`, since IDENTITY is monotonic) — computed, not stored. Users do NOT pick which shop is free (see `memory/project_free_tier_rules.md`). Two enforcement surfaces:
  - **New connections** are gated by `CanConnectAnotherShopCommand` (Domain) — returns true if Pro OR user has 0 shops. Used by all 3 Connect endpoints (Etsy/Gumroad/Stripe) AND `PlatformsViewModel`. Single source of truth — endpoints reject with `/platforms?error=plan_limit`, UI replaces the primary "Connect" button with a purple-gradient "Upgrade to Pro" CTA. UI also gates the per-platform "Connect another shop" button via `PlatformItem.CanAddAnotherShop` / `CanConnect` (both flags driven by the same command).
  - **Existing surplus shops** (e.g., user was Pro with 3 shops, lapsed to free) are handled by `GetFreeTierSlotShopIdCommand` (Domain) which returns the oldest shop id (free slot) or null for Pro/no-shops. `ShopSyncWorker` calls it before each shop sync and **skips** any shop that isn't the user's free slot (preserves user-controlled `IsEnabled`; auto-resumes on Pro upgrade with no flip-back). `PlatformsViewModel` reads it to set `ShopItem.IsLockedByFreeTier` for over-cap shops, which renders an amber "Pro-only — upgrade to resume" badge next to the health badge.
...
- **OAuth callback outcome statuses.** [EtsyCallback](Source/My.Talli.Web/Endpoints/PlatformEndpoints.cs) redirects to `/platforms?etsy={status}` (same `{platform}={status}` pattern for Gumroad/Stripe):
  - `connected` — first-time connection for this user+platform
  - `added` — existing connection, at least one new shop row inserted (fresh connect / Connect-another-shop path)
  - `refreshed` — existing shop re-authorized with the **same** account (tokens refreshed, data kept) — fires from both the Connect path and the Reconnect path
  - `swapped` — **Reconnect** path only: existing shop re-authorized with a **different** account → row re-pointed + old data hard-deleted (see "Reconnect = refresh OR swap account" rule)
...
- **Middleware whitelist.** The following always pass through even when MM is on, so the maintenance page renders, admins can call the toggle API, anonymous users can still reach the landing/sign-in flow, and asset requests don't get redirected:
  - **Exact paths:** `/` (Landing Page).
  - **Prefix matches:** `/maintenance`, `/api/admin/maintenance/*`, `/api/auth/*` (login/logout), `/signin*` (sign-in page + OAuth callbacks `/signin-google`, `/signin-microsoft`, `/signin-apple`), `/_blazor`, `/_framework`, `/css`, `/js`, `/lib`, `/Error`.
  - **Any path with a file extension** (`Path.HasExtension`) — `/app.css`, `/My.Talli.Web.styles.css`, `/favicon.svg`, `/og-image.png`, fonts, any future fingerprinted bundle. Without this, asset requests at the root path get 302-redirected to `/maintenance` and the landing page renders unstyled.
