...
- **`<Version>1.0.0.13</Version>`** in `My.Talli.Web.csproj` — single source of truth for the app version. Format: `Major.Minor.Patch.Revision`. **Initial v1.0.0.0 went live on prod 2026-05-03; subsequent revisions (v1.0.0.1–v1.0.0.13) have shipped via PR + redeploy. v1.0.0.9 (May 17) shipped free-tier enforcement (1 shop, 30-day history, no CSV export) + plan expiration lifecycle UX (banner + email + reconciliation worker). v1.0.0.10 (May 17) added BillingHealthWorker visibility + Send Subscription Expired admin button. v1.0.0.11 (May 17) pre-launch perf pass — server-side CountAsync + IsProSubscriberCommand per-circuit cache + initial nav loading bar. v1.0.0.12 (May 17) wired IDbContextFactory for true parallel queries on Export preview. v1.0.0.13 (May 17) replaced the subtle nav bar with a "Working…" modal dialog (2s minimum, MutationObserver guards the active class against Blazor's enhanced-nav DOM merge).**
...
- **Live:** `https://www.mytalli.com/` — initial **v1.0.0.0** shipped 2026-05-03 via VS Publish ZipDeploy → staging slot → slot swap. Subsequent revisions (v1.0.0.1 through **v1.0.0.13** — May 17 pre-launch perf pass + "Working…" nav dialog) have shipped via PR-to-`main` + redeploy. Every release cycle now: develop on `development_ROB`/`_ROBERT` (name alternates), PR to `main`, merge, push to Azure.
