...
- **Probe filter middleware:** Inline `app.Use(...)` block in `Program.cs`, positioned before `UseElmah()`. Short-circuits known bot/scanner paths (`.env`, `.php`, `wp-admin`, etc.) with a bare 404, OPTIONS requests with 204, and `/_blazor/disconnect` POST requests with 200 (expired circuits return 400, polluting Elmah). None of these reach Elmah, error pages, or Blazor routing.
